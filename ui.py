...
# --- Custom CSS (with new .location style) ---
st.markdown("""
<style>
    ...
    .user-card .location { font-size: 0.9rem; color: #dddddd; font-style: italic; margin-top: -10px; } /* NEW STYLE */
...
# --- Helper to reset OTP state (unchanged) ---
def reset_otp_flow():
...
if not users:
    ...
    with col1:
        ...
        # --- NEW: Get sender location ---
        sender_billing_lat = sender.get('billing_lat')
        sender_billing_long = sender.get('billing_long')
        sender_location_str = f"Lat: {sender_billing_lat}, Long: {sender_billing_long}" if sender_billing_lat is not None and sender_billing_long is not None else "Location: N/A"
        
        # --- UPDATED: Display sender card with location ---
        st.markdown(f'''<div class="user-card">
                        <h4>Sender: {sender_name}</h4>
                        <p class="balance">${sender_balance:,.2f}</p>
                        <p class="email">{sender_email}</p>
                        <p class="location">{sender_location_str}</p>
    ...
    with col2:
        ...
        # --- NEW: Get receiver location ---
        receiver_billing_lat = receiver.get('billing_lat')
        receiver_billing_long = receiver.get('billing_long')
        receiver_location_str = f"Lat: {receiver_billing_lat}, Long: {receiver_billing_long}" if receiver_billing_lat is not None and receiver_billing_long is not None else "Location: N/A"

        # --- UPDATED: Display receiver card with location ---
        st.markdown(f'''<div class="user-card">
                        <h4>Receiver: {receiver_name}</h4>
                        <p class="balance">${float(receiver["current_balance"]):,.2f}</p>
                        <p class="email">{receiver.get("email", "N/A")}</p>
                        <p class="location">{receiver_location_str}</p>
                     </div>''', unsafe_allow_html=True)

    amount = st.number_input("Amount to Transfer", min_value=0.01, step=50.0, format="%.2f", on_change=reset_otp_flow)
    
    # --- All logic below this line is completely unchanged ---
                ...
                if st.button("Email me an OTP"):
                    with st.spinner("Requesting OTP..."):
                        try:
                            ...
                            st.error("API Error: Could not connect to the API to request OTP.")
                ...
                if st.button("Verify OTP"):
                    with st.spinner("Verifying..."):
                        try:
                            ...
                            st.error("API Error: Could not connect to the API to verify OTP.")
        ...
        if amount > sender_balance:
            ...
            with st.spinner("Submitting transaction..."):
                try:
                    ...
                    st.error("API Error: Could not submit transaction.")
